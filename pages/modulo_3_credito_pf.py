"""
Módulo 3: Crédito Pessoa Física
Treemap de carteira de crédito PF por modalidade.
...
from config import (
    MODULO3_VARS, SEGMENT_COLORS, ALL_SEGMENTS, DEFAULT_SEGMENTS,
    RELATORIO_RESUMO, RELATORIO_CREDITO_PF, TIPO_INST_CREDITO,
    format_brl,
)
...
def render():
    ...
    # SESSION STATE
    # ─────────────────────────────────────────
    if "mod3_selected_var" not in st.session_state:
        st.session_state.mod3_selected_var = None
    if "mod3_segments" not in st.session_state:
        st.session_state.mod3_segments = DEFAULT_SEGMENTS.copy()
    ...
    render_module_header(
        ...
        subtitle="Ranking por modalidade de crédito PF — Carteira ativa por instituição",
    )
    ...
    var_keys = list(MODULO3_VARS.keys())
    ...
    for row_keys in [row1_keys, row2_keys]:
        ...
        for i, var_key in enumerate(row_keys):
            var_info = MODULO3_VARS[var_key]
            is_selected = st.session_state.mod3_selected_var == var_key
            ...
            with cols[i]:
                if st.button(
                    ...
                    st.session_state.mod3_selected_var = var_key
                    st.rerun()

    selected_var = st.session_state.mod3_selected_var
    selected_info = MODULO3_VARS.get(selected_var) if selected_var else None
    ...
    for i, seg in enumerate(ALL_SEGMENTS):
        with seg_cols[i]:
            is_on = seg in st.session_state.mod3_segments
            if st.checkbox(seg, value=is_on, key=f"seg3_{seg}"):
                new_segments.append(seg)

    if not new_segments:
        new_segments = ALL_SEGMENTS.copy()
    st.session_state.mod3_segments = new_segments
    ...
    anomes = find_latest_quarter(tipos=[1, 2], relatorio=RELATORIO_CREDITO_PF)

    with st.spinner(f"Carregando crédito PF ({format_anomes(anomes)})..."):
        ...
        used_tipo = None
        for tipo in [1, 2]:
            raw_df = fetch_valores(anomes, tipo=tipo, relatorio=RELATORIO_CREDITO_PF)
            if not raw_df.empty:
                used_tipo = tipo
                break

        if raw_df.empty:
            st.error("❌ Não foi possível carregar os dados de crédito PF.")
            return
        ...
        # Segment filter
        data = data[data["Segmento_Calculado"].isin(st.session_state.mod3_segments)].copy()
    ...
    st.markdown(f"""
    <div class="footer">
        Dados: IF.data — Crédito PF (Relatório 11) — {format_anomes(anomes)} ·
