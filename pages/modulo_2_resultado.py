"""
Módulo 2: Resultado (DRE)
Treemap de instituições financeiras por variáveis da Demonstração de Resultado.
Valores anualizados (soma dos últimos 4 trimestres).
"""
...
from config import (
    MODULO2_VARS, SEGMENT_COLORS, ALL_SEGMENTS, DEFAULT_SEGMENTS,
    RELATORIO_RESUMO, format_brl,
)
from data_utils import (
    fetch_valores, build_institution_table,
    extract_variable_annualized,
...
def render():
    ...
    # SESSION STATE
    # ─────────────────────────────────────────
    if "mod2_selected_var" not in st.session_state:
        st.session_state.mod2_selected_var = None
    if "mod2_segments" not in st.session_state:
        st.session_state.mod2_segments = DEFAULT_SEGMENTS.copy()
    ...
    render_module_header(
        icon="📊",
        title="Resultado (DRE)",
        subtitle="Ranking por variáveis da Demonstração de Resultado — Valores anualizados (4 trimestres)",
    )

    # ─────────────────────────────────────────
    # VARIABLE SELECTOR (2 rows: 4 + 3)
    # ─────────────────────────────────────────
    st.markdown("##### Selecione a variável de análise:")

    var_keys = list(MODULO2_VARS.keys())
    row1_keys = var_keys[:4]
    row2_keys = var_keys[4:]

    for row_keys in [row1_keys, row2_keys]:
        cols = st.columns(4)
        for i, var_key in enumerate(row_keys):
            var_info = MODULO2_VARS[var_key]
            is_selected = st.session_state.mod2_selected_var == var_key
            ...
            with cols[i]:
                if st.button(
                    ...
                    st.session_state.mod2_selected_var = var_key
                    st.rerun()

    selected_var = st.session_state.mod2_selected_var
    selected_info = MODULO2_VARS.get(selected_var) if selected_var else None
    ...
    for i, seg in enumerate(ALL_SEGMENTS):
        with seg_cols[i]:
            is_on = seg in st.session_state.mod2_segments
            if st.checkbox(seg, value=is_on, key=f"seg2_{seg}"):
                new_segments.append(seg)

    if not new_segments:
        new_segments = ALL_SEGMENTS.copy()
    st.session_state.mod2_segments = new_segments
    ...
    # ─────────────────────────────────────────
    # DATA LOADING (annualized: 4 quarters)
    # ─────────────────────────────────────────
    st.markdown("---")
    ...
    with st.spinner(f"Carregando DRE anualizado ({period_label})..."):
        ...
        # Annualized extraction (sum of 4 quarters)
        data = extract_variable_annualized(
            quarters, selected_info["relatorio"], selected_info["conta"], institutions
        )
        ...
        # Materiality filter (uses latest Resumo)
        ...
        # Absolute values for treemap
        data["Saldo_Original"] = data["Saldo"]
        data["Saldo"] = data["Saldo"].abs()

        # Segment filter
        data = data[data["Segmento_Calculado"].isin(st.session_state.mod2_segments)].copy()

        # Cleanup
        data = data.dropna(subset=["Saldo"])
        data = data[data["Saldo"] > 0].copy()
    ...
    with mcol4:
        st.metric("Período", period_label)
    ...
    st.caption("Valores anualizados (soma dos últimos 4 trimestres) · Valores absolutos")

    plot_data = data.copy()
    plot_data["Saldo_Abs"] = plot_data["Saldo"]
    ...
    st.markdown(f"""
    <div class="footer">
        Dados: IF.data — DRE (Relatório 4) — {period_label} · Anualizado (4 trimestres) ·
        Valores absolutos · Critérios: Ativos ≥ R$ 100M, PL ≥ R$ 20M, Conglomerados Prudenciais
    </div>
    """, unsafe_allow_html=True)
