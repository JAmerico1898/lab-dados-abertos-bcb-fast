"""
Módulo 6: Crédito por Região
Treemap de carteira de crédito por região geográfica.
...
from config import (
    MODULO6_VARS, SEGMENT_COLORS, ALL_SEGMENTS, DEFAULT_SEGMENTS,
...
def render():
    """Main render function for Module 6."""

    # ─────────────────────────────────────────
    # SESSION STATE
    # ─────────────────────────────────────────
    if "mod6_selected_var" not in st.session_state:
        st.session_state.mod6_selected_var = None
    if "mod6_segments" not in st.session_state:
        st.session_state.mod6_segments = DEFAULT_SEGMENTS.copy()
    ...
    render_module_header(
        ...
        subtitle="Ranking de carteira de crédito por região geográfica — Treemap por instituição",
    )

    # ─────────────────────────────────────────
    # REGION SELECTOR (single row of 5)
    # ─────────────────────────────────────────
    st.markdown("##### Selecione a região:")

    var_keys = list(MODULO6_VARS.keys())
    cols = st.columns(5)

    for i, var_key in enumerate(var_keys):
        var_info = MODULO6_VARS[var_key]
        is_selected = st.session_state.mod6_selected_var == var_key
        ...
        with cols[i]:
            if st.button(
                ...
                st.session_state.mod6_selected_var = var_key
                st.rerun()

    selected_var = st.session_state.mod6_selected_var
    selected_info = MODULO6_VARS.get(selected_var) if selected_var else None
    ...
    for i, seg in enumerate(ALL_SEGMENTS):
        with seg_cols[i]:
            is_on = seg in st.session_state.mod6_segments
            if st.checkbox(seg, value=is_on, key=f"seg6_{seg}"):
                new_segments.append(seg)

    if not new_segments:
        new_segments = ALL_SEGMENTS.copy()
    st.session_state.mod6_segments = new_segments

    # ─────────────────────────────────────────
    # GUARD
    # ─────────────────────────────────────────
    if selected_info is None:
        st.info("👆 Selecione uma região acima para visualizar os dados.")
        return
    ...
    with st.spinner(f"Carregando crédito por região ({format_anomes(anomes)})..."):
        ...
        # Fetch credit by region (Relatório 9, Tipo=1)
        ...
        # Extract by NomeColuna (standard extract_variable works here)
        ...
        # Segment filter
        data = data[data["Segmento_Calculado"].isin(st.session_state.mod6_segments)].copy()
    ...
    with mcol1:
        st.metric("Total da Região", format_brl(total))
    with mcol2:
    ...
    <tr>
        ...
        <th style="text-align:right;">CRÉDITO {selected_info['label'].upper()}</th>
        ...
        if not search_results.empty:
            ...
            st.dataframe(
                ...
                .rename(columns={
                    ...
                    "Saldo": f"Crédito {selected_info['label']}",
                    "Rank": "Posição",
                }),
                use_container_width=True,
            )
